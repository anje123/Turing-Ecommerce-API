...
class Shopping_Cart {
  ...
  static async addProductToCart(req, res) {
    ...
    //create
    const create_cart_Product = await shopping_cart.create({
      cart_id,
      product_id,
      attributes
    });
    const cartProduct = await shopping_cart.findAll({
      where: { cart_id: create_cart_Product.cart_id },
      attributes: ["item_id", "product_id", "attributes", "quantity"]
    });
    const products = await Product.findAll({
      where: { product_id: cartProduct.map(a => a.product_id) },
    ...
    for (let i = 0; i < products.length; i++) {
      ...
      const get_product_item_id = await shopping_cart.findOne({
        where: {
          product_id: product_data_values.product_id,
          cart_id: create_cart_Product.cart_id
        }
      });
      ...
      product_data_values.attributes =
        get_product_item_id.dataValues.attributes;
      product_data_values.subtotal = String(
        product_data_values.price * get_product_item_id.quantity
      );
  ...
  static async getProductCartList(req, res) {
    ...
    const products = await Product.findAll({
      where: { product_id: find_cart.map(a => a.product_id) },
      attributes: ["product_id", "name", "image", "price"]
    });
    ...
    for (let i = 0; i < products.length; i++) {
      let product_data_values = products[i].dataValues;
      const get_product_item_id = await shopping_cart.findOne({
        where: { product_id: product_data_values.product_id, cart_id: id }
      });
      product_data_values.item_id = get_product_item_id.dataValues.item_id;
      product_data_values.quantity = get_product_item_id.dataValues.quantity;
      product_data_values.attributes =
  ...
  static async updateProductCartList(req, res) {
    ...
    product_data_value.subtotal = String(
      product_data_value.price * product_data_value.quantity
    );
  ...
  static async getSavedLaterProduct(req, res) {
    ...
    const product = await sequelize.query(
      `SELECT sc.item_id, p.name, sc.attributes, COALESCE(NULLIF(p.discounted_price, 0),p.price) AS price FROM shopping_cart sc INNER JOIN product p ON sc.product_id = p.product_id WHERE sc.cart_id = ${item_id} AND NOT sc.buy_now;`
    );
    res.send(product);
  }
